...
; (function ($, window, document) {
  ...
  Calendar.prototype = {
    showCalendar: function () { // 输入数据并显示
      ...
      this.$calendarTitle_text.text(year + '年' + dateStr.split('-')[1] + '月');
      ...
      this.$calendarDate_item.each(function (i) {
        ...
        $(this).text(allDay.getDate()).attr('data', calendar.solar2lunar(allDay_str.split('-')[0], allDay_str.split('-')[1], allDay_str.split('-')[2]).lunarDate);
        if (allDay_str.split(' ')[2]) {
          $(this).attr("title", allDay_str.split(' ')[1] + '  ' + allDay_str.split(' ')[2]);
        }
        else {
          $(this).attr("title", allDay_str.split(' ')[1]);

        }
        if (returnDateStr(new Date()).split(' ')[0] === allDay_str.split(' ')[0]) {
          if (allDay_str.split(' ')[2]) {
            $(this).attr('class', 'festvals item item-curDay');
          }
          else {
            $(this).attr('class', 'item item-curDay');

          }
        } else if (returnDateStr(firstDay).split('-')[0] === allDay_str.split('-')[0] && returnDateStr(firstDay).split('-')[1] === allDay_str.split('-')[1]) {
          if (allDay_str.split(' ')[2]) {
            $(this).attr('class', 'festvals item item-curMonth');
          }
          else {
    ...
    renderDOM: function () { // 渲染DOM
      ...
      let _titleStr = '<a href="javascript:void(0)" class="title"></a>' +
        '<a href="javascript:void(0)" id="backToday">今日</a>' +
        '<div class="arrow">' +
      ...
      if (!window.matchMedia("(max-width: 1080px)").matches || window.matchMedia("(max-width: 960px)").matches) {
        this.$calendar.show();
      }
  ...
  function returnDateStr(date) { // 日期转字符串
    ...
    day = day < 9 ? ('0' + day) : ('' + day);
    let changeStr = year + '-' + month + '-' + day;

    if (calendar.getFestival()[month + '-' + day]) {
      return changeStr + ' ' + calendar.solar2lunar(year, month, day).gzYear + '年' + calendar.solar2lunar(year, month, day).IMonthCn + calendar.solar2lunar(year, month, day).IDayCn + ' ' + calendar.getFestival()[month + '-' + day]['title'];
    }
    else if (calendar.getLunarFestival()[calendar.solar2lunar(year, month, day).lMonth + '-' + calendar.solar2lunar(year, month, day).lDay]) {
      return changeStr + ' ' + calendar.solar2lunar(year, month, day).gzYear + '年' + calendar.solar2lunar(year, month, day).IMonthCn + calendar.solar2lunar(year, month, day).IDayCn + ' ' + calendar.getLunarFestival()[calendar.solar2lunar(year, month, day).lMonth + '-' + calendar.solar2lunar(year, month, day).lDay]['title'];
    }
    else {
      return changeStr + ' ' + calendar.solar2lunar(year, month, day).gzYear + '年' + calendar.solar2lunar(year, month, day).IMonthCn + calendar.solar2lunar(year, month, day).IDayCn;
    }
  };
